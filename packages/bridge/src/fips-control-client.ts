...
export type FipsControlQuery = 'peers' | 'links' | 'transports';
...
export type FipsControlData = { readonly peers: readonly FipsPeer[] } | { readonly links: readonly FipsLink[] } | { readonly transports: readonly FipsTransport[] };
...
export interface FipsControlClient { query(query: FipsControlQuery, signal?: AbortSignal): Promise<FipsControlData>; close(): Promise<void>; }
export interface FipsControlClientOptions { readonly socketPath: string; readonly connect?: StreamFactory; }

const COMMANDS: Readonly<Record<FipsControlQuery, string>> = Object.freeze({ peers: 'show_peers', links: 'show_links', transports: 'show_transports' });
...
function parseResponse(query: FipsControlQuery, bytes: Buffer): FipsControlData {
  ...
  return parseRows(query, value.data);
}
...
export function createFipsControlClient(options: FipsControlClientOptions): FipsControlClient {
  ...
  const connect = options.connect ?? ((socketPath: string): Duplex => createConnection(socketPath));
  let closed = false; let active: Duplex | undefined; let running = false;
  const queue: Array<() => void> = [];
  const drain = (): void => { if (!running && queue.length > 0) queue.shift()?.(); };
  const query = (kind: FipsControlQuery, signal?: AbortSignal): Promise<FipsControlData> => new Promise((resolve, reject) => {
    if (!(kind in COMMANDS)) { reject(new FipsControlError('query_invalid')); return; }
    ...
    const run = (): void => {
      ...
      running = true;
      const request = Buffer.from(`${JSON.stringify({ command: COMMANDS[kind] })}\n`, 'utf8');
      if (request.byteLength > MAX_REQUEST_BYTES) { running = false; reject(new FipsControlError('query_invalid')); drain(); return; }
      let socket: Duplex;
      try { socket = connect(options.socketPath); } catch { running = false; reject(new FipsControlError('transport_error')); drain(); return; }
      active = socket;
      ...
      const finish = (error?: FipsControlError, data?: FipsControlData): void => {
      ...
      const onEnd = (): void => {
        ...
        try { finish(undefined, parseResponse(kind, line)); } catch (error) { finish(error instanceof FipsControlError ? error : new FipsControlError('protocol_invalid')); }
      };
  ...
  return Object.freeze({ query, async close(): Promise<void> { if (closed) return; closed = true; active?.destroy(); while (queue.length > 0) queue.shift()?.(); } });
}
