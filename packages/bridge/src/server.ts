...
function parseBrowserAudio(payload: Record<string, unknown>): BrowserAudio {
  const keys = ['browserVersion', 'microphoneLabel', 'contextState', 'inputDeviceSampleRate', 'contextSampleRate', 'captureSampleRate', 'channels', 'echoCancellation', 'noiseSuppression', 'autoGainControl'];
...
export async function createBridgeServer(options: BridgeServerOptions): Promise<BridgeServer> {
  ...
  const buildReport = (): MachineReport | undefined => {
    ...
    const audioPassed = Boolean(audio?.microphoneLabel && audio.contextState === 'running' && (audio.inputDeviceSampleRate === 44_100 || audio.inputDeviceSampleRate === 48_000) && audio.contextSampleRate === 48_000 && audio.captureSampleRate === 48_000 && audio.channels === 1 && !audio.echoCancellation && !audio.noiseSuppression && !audio.autoGainControl);
    ...
    return { schemaVersion: 1, capturedAt: new Date().toISOString(), machine: { hostName: config.machineId, os: options.reportAuthority.build.os, architecture: options.reportAuthority.build.architecture, browserVersion: audio?.browserVersion ?? 'Unavailable', commit: options.reportAuthority.build.commit }, evidenceClass: config.evidenceClass, epoch: state.epoch, codec: { ...config.codec }, audio: audio ? { microphoneLabel: audio.microphoneLabel, contextState: audio.contextState, inputDeviceSampleRate: audio.inputDeviceSampleRate, contextSampleRate: audio.contextSampleRate, captureSampleRate: audio.captureSampleRate, channels: audio.channels, echoCancellation: audio.echoCancellation, noiseSuppression: audio.noiseSuppression, autoGainControl: audio.autoGainControl } : { microphoneLabel: 'Unavailable', contextState: 'unavailable', inputDeviceSampleRate: 0, contextSampleRate: 0, captureSampleRate: 0, channels: 0, echoCancellation: false, noiseSuppression: false, autoGainControl: false }, queues: queuesEvidence, results: canonicalResults, complete: corpusComplete, reasonCodes: [...reasons], qualification: { ...config.qualification, deadline: { ...config.qualification.deadline }, fallback: { ...config.qualification.fallback }, physicalGate }, runner: { machineId: config.machineId, role: config.role, reportTarget: config.reportTarget, evidenceClass: config.evidenceClass, tunEvidence: options.reportAuthority.tunEvidence } };
  };
