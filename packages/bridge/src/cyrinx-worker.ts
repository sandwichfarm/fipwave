...
const MIN_CAPTURE_SAMPLES = 96_000;
const MAX_CAPTURE_SAMPLES = 144_000;
...
export class CyrinxBatchWorker {
  #active: { epoch: number; generation: number; value: CyrinxCase; metadata: Buffer; startedAt: number; chunks: Buffer[]; samples: number; decoded: boolean; captureHighWaterBytes: number } | undefined;
  #generation = 0;
  #sequence = 0n;
  ...
  async begin(value: CyrinxCase, epoch: number): Promise<Buffer> {
    ...
    this.#active = { epoch, generation, value, metadata: meta, startedAt, chunks: [], samples: 0, decoded: false, captureHighWaterBytes: 0 };
  ...
  async receiveCapture(encoded: Buffer): Promise<CyrinxResult | undefined> {
    ...
    const frame = decodeFrame(encoded); if (frame.type !== MessageType.PCM_CAPTURE || frame.epoch !== active.epoch || frame.channels !== 1 || frame.sampleRate !== 48_000) return this.reject('cyrinx_capture_invalid');
    const pcm = decodePcmPayload(frame.payload, 1); const samples = pcm.samples.byteLength / 4;
    if (active.samples + samples > MAX_CAPTURE_SAMPLES) return this.reject('cyrinx_capture_overflow');
    active.chunks.push(pcm.samples); active.samples += samples; active.captureHighWaterBytes = Math.max(active.captureHighWaterBytes, active.samples * 4);
    if (active.samples < MIN_CAPTURE_SAMPLES) return undefined;
    ...
    const digest = createHash('sha256').update(body.subarray(289)).digest('hex'); const now = (this.options.now ?? Date.now)(); const result: CyrinxResult = { caseId: active.value.id, digest, acquisitionMs: Math.max(0, now - active.startedAt), airtimeMs: body.readUInt32LE(21), complete: digest === active.value.digest, corrupt: digest !== active.value.digest, missing: 0, duplicates: 0, deliveryCount: 1, bytePerfect: digest === active.value.digest, coldAcquired: true, queues: { captureHighWaterBytes: active.captureHighWaterBytes, captureHighWaterMs: active.samples / 48, playbackHighWaterBytes: PCM_BYTES, playbackHighWaterMs: FRAME_SAMPLES / 48, discontinuities: 0 } };
