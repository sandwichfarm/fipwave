...
export function checkFipsComposeSource(composeSource, bridgeDockerfile, fipsDockerfile) {
  ...
  if (!bridgeDockerfile.includes('socat')) fail('bridge image must provide the internal loopback forwarder');
  ...
  if (!composeSource.includes('--port 4311') || !composeSource.includes('TCP-LISTEN:4310,fork,reuseaddr TCP:127.0.0.1:4311')) fail('bridge must forward its published port only to the internal loopback runner');
