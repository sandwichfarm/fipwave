...
export function checkFipsComposeSource(composeSource, bridgeDockerfile, fipsDockerfile) {
  ...
  if (!composeSource.includes('--port 4310') || composeSource.includes('socat') || composeSource.includes('TCP-LISTEN:')) fail('bridge must serve its published loopback port directly');
